...
## 3. 本番の手順（2026-09-22 0 時台に実施。当初予定は 9/24）
...
| 手順 | 結果 | 時間 |
|---|---|---|
| 1. 列と索引 | 4 索引＋`customer_code` | 2 秒 |
| 3〜5. 過去分の取り込み | 1,506,611 行 → 取消 2,879 行を除いて 1,503,732 品目 | 4 分 14 秒 |
| 6. 返却 CSV の再反映 | 返却日時を入れた品目 1,319,699 | 9 分 58 秒 |
| 7. 返却日時列で補う | 1,845 品目 | 2 分 2 秒 |
| 8. 顧客コード | 既存 255,136 件中 253,670 件 | 1 分 18 秒 |

- 返却記録なし: 184,652 品目（タグ有 60,422・タグ無し 124,230。タグ無しは更新・券・新規入会など品物でない行）
- 受け入れ確認（依頼書 §6 と追記の速さの条件）すべて合格。`/customers/history` は存在しない氏名 0.03 秒、全期間 9,137 品目の会員 0.14 秒（nginx 経由）
- バックアップ 19MB・7 秒 → 54MB・19 秒。スワップ 435MB → 562MB
- 請求連携を `--since 2025-10-01` で流し直し、2025-10 初めの請求 4 件が 2025-09 の品目に紐付いた（受付日の 30 日前〜7 日後の規則どおり）
